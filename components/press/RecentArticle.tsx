...
import { RECENT_ARTICLES_FOR_PRESS_QUERY } from "@/lib/sanity/queries";
...
async function RecentArticle({ pressPageData }: RecentArticleProps) {
  ...
  const recentArticles = await client.fetch(RECENT_ARTICLES_FOR_PRESS_QUERY);
      ...
      <MotionWrapper variants={section}
        ...
        <MotionWrapper variants={grid} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.35 }}>
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8'>
            {recentArticles.slice(0, config.limit).map((article: Article) => (
