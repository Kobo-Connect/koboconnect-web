...
type AssetImage = { asset: { url: string; metadata?: { dimensions?: { width: number; height: number } } }; alt?: string };
...
export default function Hero({ data }: { data?: HeroData }) {
    const displayTitle = data?.title || 'Banking Without Walls\nSecurity Without\nCompromise';
    const displayDescription = data?.description || 'We built Kobo Vault to bring financial freedom closer to everyone. With advanced money transfer options, automated payments, and smart virtual cards, you can transact safely and confidently.';
            ...
            <div className='max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 py-4 md:py-6 px-4 relative z-10'>
                <div className='col-span-2 max-w-2xl  py-10 text-white'>
                    ...
                    <div className='flex gap-5 pt-4'>
                        ...
                        {data?.playStoreLink ? (
                            <a href={data.playStoreLink} target='_blank' rel='noopener noreferrer'>
                                <Image src={playStoreSvg} alt='play-store' width={150} height={150} priority />
                            </a>
                        ) : (
                            <Image src={playStoreSvg} alt='play-store' width={150} height={150} priority />
                        )}
                ...
                <div className='col-span-1 min-h-[500px]'>
                    {heroUrl ? (
                        <Image src={heroUrl} alt={data?.heroImage?.alt || 'hero-img'} width={500} height={500} priority />
                    ) : (
