...
async function Footer({ data }: FooterProps) {
  ...
  return (
    <div className='bg-[#0A1B1B]'>
      <MotionWrapper as="section" initial="hidden"
        whileInView="show"
        viewport={{ once: true, amount: 0.25 }}>
        <div className='max-w-7xl mx-auto pt-8 md:pt-14 px-4 relative'>
            ...
            <div className='flex flex-col gap-4 '>
              ...
              <div className='flex gap-4 mt-4'>
                {displayData.socialLinks.map((social) => (
                  <MotionWrapper key={social.name} whileHover={{ y: -2, scale: 1.03 }}
                    whileTap={{ scale: 0.98 }}
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 gap-6'>
              {displayData.footerLinks.map((link) => (
                <div key={link.title}>
                  ...
                  <ul>
                    {link.links.map((item) => (
                      <MotionWrapper key={item.title} as="li" variants={{
